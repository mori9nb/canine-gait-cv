...
def test_load_superanimal_json_maps_native_hind_limb_names(tmp_path) -> None:
    ...
    assert frame.dog_box == BoundingBox(10, 20, 111, 221)
...
def test_load_superanimal_json_rejects_wrong_bodypart_count(tmp_path) -> None:
    ...
    expected_count = len(SUPERANIMAL_QUADRUPED_BODY_PARTS)

    with pytest.raises(
        ValueError,
        match=rf"expected {expected_count} bodyparts",
    ):
...
def test_load_superanimal_multi_json_preserves_all_individuals(
    tmp_path,
) -> None:
    frame = _frame()
    frame["bodyparts"].append(
        _points(
            x_offset=100.0,
            y_offset=200.0,
            confidence=0.8,
    ...
    frame["bboxes"].append([300.0, 50.0, 200.0, 200.0])
    ...
    assert (
        frames[0].individuals[1].dog_box
        == BoundingBox(300, 50, 500, 250)
    )
    assert (
        frames[0]
        .individuals[1]
        .pose.get(KeypointName.NOSE)
        .x
        == 100.0
    )
...
def test_multi_loader_rejects_mismatched_individual_arrays(
    tmp_path,
) -> None:
    frame = _frame()
    frame["bbox_scores"] = []
    ...
    with pytest.raises(
        ValueError,
        match="same number of individuals",
    ):
        load_superanimal_multi_json(path)


def test_load_superanimal_multi_json_skips_empty_dlc_slots(
    tmp_path,
) -> None:
    empty_points = [
        [-1.0, -1.0, -1.0]
        for _ in SUPERANIMAL_QUADRUPED_BODY_PARTS
    ]
    ...
    assert len(frames) == 1
    assert len(frames[0].individuals) == 1
    assert (
        frames[0].individuals[0].bbox_confidence
        == pytest.approx(0.95)
    )